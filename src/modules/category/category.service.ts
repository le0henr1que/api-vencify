...
@Injectable()
export class CategoryService implements IBaseService<CategoryTypeMap> {
  ...
  async createAsync(
    ...
    this.logger.log(`${identifierRequest} Create category`);

    const executeCreate = async (transaction: Prisma.TransactionClient) => {
      // TODO-GENERATOR: INSERT THE CREATE ENTITY LOGIC
      const dataCreate: CategoryTypeMap[CrudType.CREATE_UNCHECKED] = {
        name: '',
        supplier_id: '',
      };
      return await this.categoryRepository.createAsync(dataCreate, transaction);
    };
  ...
  async updateAsync(
    ...
    const identifierRequest = optionals?.identifierRequest || randomUUID();
    this.logger.log(`${identifierRequest} Update category`);
    ...
    const executeUpdate = async (transaction: Prisma.TransactionClient) => {
      ...
      if (
        ...
        this.logger.debug(`${identifierRequest} category not found`);
        throw new NotFoundException(
          getMessage(MessagesHelperKey.NOT_FOUND, languagePreference),
        );
      }

      // TODO-GENERATOR: Insert the data received in the CategoryUpdateInput
      const categoryUpdateInput: CategoryTypeMap[CrudType.UPDATE] = {
        version: data.version,
      };

      return await this.categoryRepository.updateAsync(
        id,
        categoryUpdateInput,
        transaction,
      );
  ...
  async deleteAsync(
    ...
    const identifierRequest = optionals?.identifierRequest || randomUUID();
    this.logger.log(`${identifierRequest} Delete category`);
    ...
    const executeDelete = async (transaction: Prisma.TransactionClient) => {
      ...
      await this.categoryRepository.deleteAsync(id, version, transaction);
    };
  ...
  async findByIdAsync<S, T>(
    ...
    try {
      ...
      const data = await this.categoryRepository.findByIdAsync(
        id,
  ...
  async findBy<S, T>(
    ...
    try {
      ...
      const data = await this.categoryRepository.findBy(
        where,
        select,
        optionals?.orderBy,
        optionals?.transaction,
  ...
  ): Promise<Partial<CategoryEntity>[] | T[]> {
    const identifierRequest = optionals?.identifierRequest || randomUUID();
    try {
      ...
      const data = await this.categoryRepository.findAllBy(
        where,
        select,
        optionals?.orderBy,
        optionals?.transaction,
  ...
  ): Promise<Paginated<Partial<CategoryPaginationResponse>>> {
    const identifierRequest = optionals?.identifierRequest || randomUUID();
    try {
      this.logger.log(`${identifierRequest} Find filtered async`);

      return await this.categoryRepository.findFilteredAsync(
        filter,
        currentUser,
        optionals?.transaction,
      );
