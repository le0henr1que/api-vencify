...
import {
  generateCreateInput,
  generateUpdateInput,
} from './fixtures/supplier/supplier.fixture.generator';
import { revertSupplierToInitialState } from './fixtures/supplier/supplier.test-utils';
import { authenticateUser } from './shared/auth.shared';
import { baseAuthenticationTests } from './shared/base.tests';

describe('Supplier module E2E Tests', () => {
  ...
  beforeEach(async () => {
    if (supplierId) {
      await revertSupplierToInitialState(prisma, supplierId);
    }
  });

  describe('Create Supplier', () => {
    const endPointUrl = '/supplier';
  ...
  describe('Get filtered Supplier', () => {
    const endPointUrl = '/supplier';
  ...
  describe('Get Supplier by id', () => {
    ...
    it('Should throw error if Supplier id doesnt exist', async () => {
  ...
  describe('Delete Supplier', () => {
    ...
    it('Should inactive a Supplier', async () => {
      await request(app.getHttpServer())
        .delete(`${endPointUrl}/${supplierId}`)
        .query({ version: 1 })
        .set('Authorization', `Bearer ${bearerToken}`)
        .expect(200);
    });
