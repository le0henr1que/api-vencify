...
import {
  generateCreateInput,
  generateUpdateInput,
} from './fixtures/category/category.fixture.generator';
import { revertCategoryToInitialState } from './fixtures/category/category.test-utils';
import { authenticateUser } from './shared/auth.shared';
import { baseAuthenticationTests } from './shared/base.tests';

describe('Category module E2E Tests', () => {
  ...
  beforeEach(async () => {
    if (categoryId) {
      await revertCategoryToInitialState(prisma, categoryId);
    }
  });
  ...
  describe('Get filtered Category', () => {
    const endPointUrl = '/category';
  ...
  describe('Get Category by id', () => {
    ...
    it('Should throw error if Category id doesnt exist', async () => {
  ...
  describe('Delete Category', () => {
    ...
    it('Should inactive a Category', async () => {
      await request(app.getHttpServer())
        .delete(`${endPointUrl}/${categoryId}`)
        .query({ version: 1 })
        .set('Authorization', `Bearer ${bearerToken}`)
        .expect(200);
    });
