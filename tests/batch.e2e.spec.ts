...
import {
  generateCreateInput,
  generateUpdateInput,
} from './fixtures/batch/batch.fixture.generator';
import { revertBatchToInitialState } from './fixtures/batch/batch.test-utils';
import { authenticateUser } from './shared/auth.shared';
import { baseAuthenticationTests } from './shared/base.tests';

describe('Batch module E2E Tests', () => {
  ...
  beforeEach(async () => {
    if (batchId) {
      await revertBatchToInitialState(prisma, batchId);
    }
  });

  describe('Create Batch', () => {
    const endPointUrl = '/batch';
  ...
  describe('Get Batch by id', () => {
    ...
    it('Should throw error if Batch id doesnt exist', async () => {
  ...
  describe('Delete Batch', () => {
    ...
    it('Should inactive a Batch', async () => {
      await request(app.getHttpServer())
        .delete(`${endPointUrl}/${batchId}`)
        .query({ version: 1 })
        .set('Authorization', `Bearer ${bearerToken}`)
        .expect(200);
    });
